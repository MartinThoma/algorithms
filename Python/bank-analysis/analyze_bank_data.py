...
def read_data(csv_filepath) -> pd.DataFrame:
    df = pd.read_csv(csv_filepath, parse_dates=["Datum"])
    assert "Empfänger" in df.columns
    assert "Betrag (EUR)" in df.columns
    ...
    mapping["stripe payments uk ltd"] = "Stripe"
    mapping["stripe technology europe ltd"] = "Stripe"
...
if __name__ == "__main__":
    df = read_data("n26-csv-transactions.csv")
    print(df)
